...
def compute_trust_scores(student_id, user_number, phase, dict_df=None):
    ...
    conn.close()
    if not qdf.empty:
        # Filter out None values in case of bad data, though validation should prevent this
...
def phase_ui_common(phase_num: int, dict_df: pd.DataFrame):
    ...
    if submit_btn and not st.session_state.get(f"phase{phase_num}_done", False):
        # VALIDATION: Check for empty inputs before saving
        errors = []
        if decision is None:
            errors.append("Please select Yes or No for Insider Threat.")
        ...
        if errors:
            ...
            try:
                ...
                if not correct_row.empty:
                    correct_scenario = int(correct_row.iloc[0]["scenario"])
                    # If decision is No, scenario is -1, marks 50 if correct is 0.
                    # If decision is Yes, scenario must match.
                    if decision == "No":
                         scenario_marks = 50 if correct_scenario == 0 else 0
                         insider_marks = 50 if correct_scenario == 0 else 0 # Correctly identified not an insider
                    else:
                         scenario_marks = 50 if int(scenario) == correct_scenario else 0
                         insider_marks = 50 if correct_scenario != 0 else 0 # Correctly identified as insider
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        with st.expander("Debug Dictionary"):
            if dict_df is not None:
                st.dataframe(dict_df.head())
            else:
                st.write("Dictionary not loaded")
