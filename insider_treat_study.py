    ...
    if phase_num == 2:
        ...
        with tab1:
            ...
            if os.path.exists(path):
                ...
                st.image(path, width=300)   # Change width here too
            else:
        ...
        with tab2:
            ...
            if files:
                ...
                for f in files:
                    ...
                    st.image(f, width=300)   # Change width here too
            else:
...
def main():
    ...
    # -------------------- PDF GENERATION (OPTION A: last phase_time) --------------------
    if 'generate_report' in locals() and generate_report:
        # pull student_id and user_numbers from session_state (if available)
        ...
        # choose a user_number to show in header (prefer phase1 if present)
        display_user_number = user_number_p1 if user_number_p1 is not None else user_number_p2

        # compute trust scores for both phases (uses compute_trust_scores)
        t1 = compute_trust_scores(student_id, user_number_p1, 1, ground_truth_df) if (student_id is not None and user_number_p1 is not None) else {"objective": None, "subjective": None, "combined": None}
        t2 = compute_trust_scores(student_id, user_number_p2, 2, ground_truth_df) if (student_id is not None and user_number_p2 is not None) else {"objective": None, "subjective": None, "combined": None}

        # helper: get last recorded phase_time for (student_id, user_number, case_type)
        ...
        # compute differences and interpretation
        trust_diff = None
        ...
        elements.append(Paragraph(f"Student ID: <b>{student_id}</b>", styles["Normal"]))
        elements.append(Paragraph(f"User Number: <b>{display_user_number}</b>", styles["Normal"]))
        ...
        elements.append(Spacer(1, 0.2 * inch))
            ...
            # -------- Justification for Trust Increase/Decrease --------
        if trust_diff is not None:
            if trust_diff > 0:
                trust_justification = (
                    ...
                    "Prior work in explainable AI shows that interpretable feature-attribution methods "
                    "enhance user confidence, reduce uncertainty, and improve perceptions of model "
                    "competence and fairness. Consequently, analysts often trust the model more once they "
                    "have visibility into its decision rationale."
                )
            elif trust_diff < 0:
                trust_justification = (
                    "The decrease in trust during Phase 2 is also theoretically grounded. When participants "
                    "review SHAP-based explanations, they may detect inconsistencies, unexpected feature "
                    "weights, or reasoning patterns that do not match their domain knowledge. Exposure to "
                    "these explanations can highlight model weaknesses that were not visible in Phase 1. "
                    "Research in explainable AI indicates that trust can decrease when explanations reveal "
                    "limitations or questionable decision logic, reflecting constructive skepticism. "
                    "Therefore, reduced trust does not necessarily indicate misunderstanding—rather, it "
                    "shows that participants critically evaluated the explanations and adjusted their trust accordingly."
                )
            else:
                trust_justification = (
            ...
            # -------- Justification for Increased Time --------
        if time_diff is not None and time_diff > 0:
            justification_text = (
                "The increase in time during Phase 2 is expected. "
                "In Phase 2, participants were required to interpret SHAP-based explainability graphs, "
                "which introduces additional cognitive processing. Unlike Phase 1—where participants only "
                "observed raw anomaly scores—Phase 2 involves examining feature-level attributions, "
                "understanding why the model marked specific events as anomalous, and validating these "
                "explanations against their mental model of insider threat behavior. "
                "Prior research in explainable AI shows that interpretability artefacts increase cognitive load "
                "and generally lead to longer decision-making times because users must integrate model "
                "rationales with the classification outcome. "
                "Therefore, the additional time reflects deeper analytical reasoning rather than inefficiency."
            )
            elements.append(Spacer(1, 0.2 * inch))
        ...
        # Provide download button in sidebar
