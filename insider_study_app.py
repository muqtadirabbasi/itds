        ...
        if os.path.exists(scores_path):
            try:
                ...
                if 'score' in df_scores.columns:
                    sorted_scores = df_scores['score'].sort_values()
                    cdf_vals = sorted_scores.rank(method="max") / len(sorted_scores)
                    fig, ax = plt.subplots()
                    ax.plot(sorted_scores, cdf_vals, marker='.')
                    ax.set_xlabel("Anomaly Score")
                    ax.set_ylabel("CDF")
                    ax.set_title("CDF of Anomaly Scores")
                    st.pyplot(fig)
                else:
                    st.info("Column 'score' missing in anomaly scores CSV.")
            except Exception as e:
                st.warning(f"Failed to read scores CSV: {e}")
        else:
