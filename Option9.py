...
# Timezone setup - Safe fallback if zoneinfo is missing data
try:
...
def load_dictionary_scenario(path: str = DICTIONARY_SCENARIO_CSV) -> pd.DataFrame:
    ...
    # Using python engine and sep=None to auto-detect delimiters
    ...
    if "user_number" in df.columns:
        # Force numeric, coerce errors to NaN, then fill with 0 or drop
...
def build_pdf_report_bytes_comparative(
    ...
    header_style = ParagraphStyle(
        ...
        fontSize=14, spaceBefore=12, spaceAfter=10,
        textColor=colors.darkblue
    )
    ...
    elements.append(Paragraph(f"Generated on: {now_toronto().strftime('%Y-%m-%d %H:%M')}", ParagraphStyle(
        "Date", parent=normal, alignment=TA_CENTER
    )))
...
def phase_ui_common(phase_num: int, dict_df: pd.DataFrame):
    ...
    with c3:
        ...
        # Removed 'key' parameter to fix update issue
    ...
    # ---------------------------------------------------------
    # 2. PLOTS (Anomaly Loss & CDF) - RESIZED
    # ---------------------------------------------------------
    st.markdown("**2. Visual Analysis Plots**")
    
    col_plot1, col_plot2 = st.columns(2)
    
    # Define standard size for consistency (Width, Height)
    ...
    with col_dec1:
        ...
        # Scenario dropdown visible ONLY when decision is Yes
        if decision == "Yes":
    ...
    with col_dec2:
        ...
        # Scenario Confidence slider visible ONLY when decision is Yes
        if decision == "Yes":
            ...
            # Default value when hidden so calculation below doesn't break
    ...
    if submit_btn and not st.session_state.get(f"phase{phase_num}_done", False):
        ...
        # Objective marks (scenario correctness + insider correctness)
        obj_marks = None
        try:
            correct_row = dict_df[dict_df["user_number"] == int(user_number)]
            if not correct_row.empty:
