...
# CERT raw input CSV loader
@st.cache_data
def load_raw_input_csv(path: str) -> pd.DataFrame:
    """
    Parse CERT r5.2 per-user event logs where each line is:

        event_type,event_id,timestamp,user,pc,<arbitrary text with commas>

    We:
      - split on at most 5 commas -> 6 fields
      - map them to:
          [event_type, event_id, timestamp, user, pc, details]
      - any extra commas stay inside 'details'.
    """
    rows = []
    with open(path, "r", errors="replace") as f:
        for line in f:
            ...
            if not line:
                continue
            parts = line.split(",", 5)   # at most 6 pieces
            if len(parts) < 6:
                parts += [""] * (6 - len(parts))
            rows.append(parts)
...
# Only insider scenarios 1–4 are selectable by the user.
...
SCENARIO_LABELS = {
    ...
    -1: "N/A (No Potential Insider Selected)",   # internal / disabled only
     0: "0 - Normal User (Not Selectable)",      # kept for compatibility, not shown
}
...
def _extract_confusion_for_user(ground_truth_df, user_number):
    ...
    for c in ground_truth_df.columns:
        cl = c.strip().lower()
        if cl == 'tp' or 'true positive' in cl:
            mapping['TP'] = c
        if cl == 'tn' or 'true negative' in cl:
            mapping['TN'] = c
        if cl == 'fp' or 'false positive' in cl:
            mapping['FP'] = c
        if cl == 'fn' or 'false negative' in cl:
            mapping['FN'] = c
    # fallback
...
def compute_trust_scores(student_id, user_number, phase, ground_truth_df=None, dict_df=None):
    ...
    # objective
    # NOTE: case_type strings kept for DB compatibility (not shown to user)
...
def phase_ui_common(phase_num, ground_truth_df, dict_df):
    ...
    with c1:
        ...
        student_id = st.number_input("Participant ID", min_value=1, max_value=9999,
                                     value=int(default_student), key=f"sid_p{phase_num}")
        st.session_state['student_id'] = student_id
    with c2:
        user_number = st.number_input("Suspect User ID", min_value=0, max_value=9999,
                                      value=0, key=f"user_p{phase_num}")
    ...
    with col_conf:
        ...
        if not ground_truth_df.empty:
            ...
            if conf_temp:
                ...
                cm = [[conf_temp['TP'], conf_temp['FN']],
                      [conf_temp['FP'], conf_temp['TN']]]
                sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                            xticklabels=["Pred Yes","Pred No"],
                            yticklabels=["Actual Yes","Actual No"], ax=ax)
                st.pyplot(fig)
            else:
                st.info("Data unavailable.")
        else:
            st.info("Ground truth missing.")
    with col_cdf:
    ...
    # --- Input & Output Data (CSV + logs, via expanders) ---
    st.markdown("---")
    st.markdown("**5. Model Input and Output Data**")

    # Input CSV (by username, r5.2-* folders)
            ...
            if not matches:
                ...
                if len(matches) == 1:
                    ...
                    chosen_path = st.selectbox(
                        ...
                        options=matches,
                        format_func=lambda p: os.path.join(os.path.basename(os.path.dirname(p)),
                                                          os.path.basename(p)),
        ...
        try:
            ...
            if testing_index != -1:
                ...
                if entries:
                    ...
                    df_user = df_log[df_log['User'] == user_number].reset_index(drop=True)
                    if not df_user.empty:
                        # hide Ans, rename Pred -> Predictions
                        df_view = df_user.drop(columns=['Ans'])
                        df_view = df_view.rename(columns={'Pred': 'Predictions'})
                        # format Loss with fixed decimal places so small values are visible
                        if 'Loss' in df_view.columns:
                            st.dataframe(
                                df_view.style.format({'Loss': '{:.6f}'}),
    ...
    with col_dec1:
        decision = st.radio(
            ...
            ["No","Yes"], index=0,
            key=f"p{phase_num}_decision", horizontal=True
        )
        if decision == "No":
            # Scenario not applicable when there is no potential insider
            scenario = -1
            st.selectbox(
                "Scenario Type (N/A when 'No')",
                [-1],
                format_func=scenario_label,
                index=0,
                disabled=True,
                key=f"p{phase_num}_scenario_no"
            )
        else:
            # Only insider scenarios 1–4 are selectable
    ...
    submit_btn = st.button(f"Submit {design_label}",
                           key=f"submit_p{phase_num}", type="primary")
    ...
    if submit_btn and not st.session_state.get(f'phase{phase_num}_done', False):
        ...
        # Combine decision + scenario confidence into a single 0–100 score
        confidence_marks = ((decision_confidence - 1) + (scenario_confidence - 1)) / 2.0 * 25.0
        confidence_text = (
            f"Decision confidence: {decision_confidence}/5; "
            f"Scenario confidence: {scenario_confidence}/5."
        )

        # Internal labels kept for DB compatibility
        case_type = "Without Explainability" if phase_num==1 else "With Explainability"

        try:
            save_response(
                student_id, user_number, scenario, decision,
                confidence_text, case_type,
...
def main():
    ...
    if generate_report:
        ...
        if t1.get("combined") and t2.get("combined"):
            ...
            if diff > 0:
                trust_change = "Trust Increased in Design B"
            elif diff < 0:
                trust_change = "Trust Decreased in Design B"
        
        summary_text = f"""
        <b>Trust Trend:</b> {trust_change}.<br/><br/>
        <b>Analysis:</b> Design B introduces SHAP-based explainability support compared to Design A. 
        An increase in trust typically indicates that the additional explanations reinforced the participant's confidence
        in the model's reasoning. A decrease may indicate that the explanations highlighted potential inconsistencies
        or misalignments with the participant's own judgment.
        """
    ...
    if page_choice == "Instructions":
        st.markdown(
            ...
            "2. For each design (A and B), review the evidence (loss, ROC, confusion, CDF, input and output data).\n"
            "3. Make your decision, select scenario and confidence.\n"
