...
def load_dictionary_scenario(path: str = DICTIONARY_SCENARIO_CSV) -> pd.DataFrame:
    ...
    if "user_number" in df.columns:
        df["user_number"] = pd.to_numeric(df["user_number"], errors="coerce").fillna(0).astype(int)
        
    if "scenario" in df.columns:
        df["scenario"] = pd.to_numeric(df["scenario"], errors="coerce").fillna(-1).astype(int)
...
def compute_trust_scores(student_id, user_number, phase, dict_df=None):
    # subjective
    subj = None
    conn = sqlite3.connect(DB_FEEDBACK)
    ...
    conn.close()
    if not qdf.empty:
        # Filter out None values in case of bad data
        valid_vals = qdf.iloc[0].dropna().astype(float)
        if not valid_vals.empty:
            subj = round(float(valid_vals.mean() * 20.0), 2)
    ...
    if user_response and dict_df is not None and not dict_df.empty:
        try:
            ...
            if not correct_row.empty:
                ...
                # Check based on new labels
                ans_str = str(user_response.get("answer", "")).strip()
                if "Potential Insider Threat" in ans_str and "Not" not in ans_str:
                    user_insider = True
                elif "Not a Potential" in ans_str:
                    user_insider = False
                else:
                    user_insider = None
...
def get_last_response_value(student_id: int, user_number: int, phase: int, col: str):
    case_type = "Without Explainability" if phase == 1 else "With Explainability"
    conn = sqlite3.connect(DB_RESPONSES)
    try:
        df = pd.read_sql_query(
            f"SELECT {col} FROM responses WHERE student_id=? AND user_number=? AND case_type=? ORDER BY timestamp DESC LIMIT 1",
...
def build_pdf_report_bytes_comparative(
    ...
    title_style = ParagraphStyle("MainTitle", parent=styles["Heading1"], fontSize=18, alignment=TA_CENTER, spaceAfter=18, textColor=colors.darkblue)
    header_style = ParagraphStyle("Header2", parent=styles["Heading2"], fontSize=14, spaceBefore=12, spaceAfter=10, textColor=colors.darkblue)
    normal = styles["Normal"]

    elements = []
    elements.append(Paragraph("Insider Threat Assessment Report", title_style))
    elements.append(Paragraph(f"Generated on: {now_toronto().strftime('%Y-%m-%d %H:%M')}", ParagraphStyle("Date", parent=normal, alignment=TA_CENTER)))
    elements.append(Spacer(1, 0.25*inch))

    display_user = user_a if user_a is not None else user_b
    info_data = [
        ["Participant ID", str(student_id) if student_id is not None else "N/A"],
        ["Suspect User ID", str(display_user) if display_user is not None else "N/A"],
        ["Assessment Date", now_toronto().strftime("%Y-%m-%d")]
    ]
    info_table = Table(info_data, colWidths=[2*inch, 3.2*inch], hAlign="LEFT")
    ...
    t1 = {}
    t2 = {}
    p1_time = p2_time = None
    p1_obj = p2_obj = None
    p1_conf = p2_conf = None

    if student_id is not None and user_a is not None:
        t1 = compute_trust_scores(student_id, user_a, 1, dict_df=dict_df)
        p1_time = get_last_response_value(student_id, user_a, 1, "phase_time")
        p1_obj = get_last_response_value(student_id, user_a, 1, "marks")
        p1_conf = get_last_response_value(student_id, user_a, 1, "confidence_marks")

    if student_id is not None and user_b is not None:
        t2 = compute_trust_scores(student_id, user_b, 2, dict_df=dict_df)
        p2_time = get_last_response_value(student_id, user_b, 2, "phase_time")
        p2_obj = get_last_response_value(student_id, user_b, 2, "marks")
        p2_conf = get_last_response_value(student_id, user_b, 2, "confidence_marks")

    elements.append(Paragraph("Comparative Analysis: Design A vs Design B", header_style))

    comp_data = [
        ["Metric", "Design A", "Design B", "Difference"],
        ["Objective Trust", t1.get("objective", "-"), t2.get("objective", "-"), _diff_str(t1.get("objective"), t2.get("objective"))],
        ["Subjective Trust", t1.get("subjective", "-"), t2.get("subjective", "-"), _diff_str(t1.get("subjective"), t2.get("subjective"))],
        ["Combined Trust", t1.get("combined", "-"), t2.get("combined", "-"), _diff_str(t1.get("combined"), t2.get("combined"))],
        ["Performance Marks", p1_obj if p1_obj is not None else "-", p2_obj if p2_obj is not None else "-", _diff_str(p1_obj, p2_obj)],
        ["Confidence (0-100)", p1_conf if p1_conf is not None else "-", p2_conf if p2_conf is not None else "-", _diff_str(p1_conf, p2_conf)],
        ["Time Taken (sec)", f"{float(p1_time):.2f}" if p1_time is not None else "-", f"{float(p2_time):.2f}" if p2_time is not None else "-", _diff_str(p1_time, p2_time)],
    ]

    t = Table(comp_data, colWidths=[2.1*inch, 1.5*inch, 1.5*inch, 1.1*inch])
    ...
    try:
        if t1.get("combined") is not None and t2.get("combined") is not None:
            diff = float(t2["combined"]) - float(t1["combined"])
            if diff > 0:
                trust_change = "Trust Increased (Design B demonstrated a positive impact)."
            elif diff < 0:
                trust_change = "Trust Decreased (Design B induced skepticism or uncertainty)."
    except Exception:
        pass

    summary_text = (
        f"<b>Trust Trend:</b> {trust_change}<br/><br/>"
        f"<b>Interpretation:</b> Differences between Design A and Design B can be driven by the availability "
        f"and clarity of evidence (e.g., explainability plots in Design B), cognitive load, and the user’s ability "
        f"to validate anomalies against behavioral signals. Increased trust is often associated with improved perceived "
        f"transparency and decision assurance, whereas decreased trust can reflect mismatch between model outputs and human intuition."
    )
...
def phase_ui_common(phase_num: int, dict_df: pd.DataFrame):
    ...
    with c3:
        ...
        if dict_df is None or dict_df.empty:
            st.caption("⚠️ Dictionary CSV not loaded or empty.")
        else:
            try:
                ...
                st.caption(f"Error looking up user: {e}")
        st.text_input("User Name", value=username_display, disabled=True) 

    st.divider()

    # --- Timer ---
    if not st.session_state.get(f"phase{phase_num}_done", False):
        if f"p{phase_num}_start" not in st.session_state:
    ...
    with st.expander("View Model Output Logs", expanded=True):
        ...
        try:
            ...
            st.info("Log file unreadable or missing.")

    st.markdown("---")

    # 2. PLOTS
    st.markdown("**2. Visual Analysis Plots**")
    ...
    img_loss = load_resized_image(loss_path, STANDARD_SIZE)
    img_cdf = load_resized_image(scores_path, STANDARD_SIZE)
    
    with col_plot1:
        st.markdown("*Anomaly Loss*")
        if img_loss:
            st.image(img_loss, use_container_width=True)
        else:
            st.warning("Anomaly Image not found.")
    with col_plot2:
        st.markdown("*CDF Curve*")
        if img_cdf:
            st.image(img_cdf, use_container_width=True)
        else:
            st.info("CDF Image not available.")

    st.markdown("---")

    # 3. INPUT LOGS
    st.markdown("**3. Raw Input Logs (Activity Data)**")
    with st.expander("View Input CSV Data", expanded=False):
    ...
    # 1. Decision Radio: No default (index=None)
    with col_dec1:
        # UPDATED QUESTION & OPTIONS
        decision = st.radio(
            "What is your decision regarding this user’s behavior?",
            ["Not a Potential Insider Threat", "Potential Insider Threat"],
            index=None,  # No default value
            key=f"p{phase_num}_decision",
            horizontal=False, # Changed to False for longer labels
        )

        if decision == "Potential Insider Threat":
            # 2. Scenario Selectbox: No default (index=None)
    ...
    # 3. Confidence Inputs: Switched to Radio for "No Default" capability
    with col_dec2:
        decision_confidence = st.radio(
        ...
        if decision == "Potential Insider Threat":
            scenario_confidence = st.radio(
                "Scenario Confidence (1-5)",
                options=[1, 2, 3, 4, 5],
                index=None,  # No default value
                format_func=lambda x: f"{x} - {conf_labels[x]}",
            ...
            scenario_confidence = 1 # Dummy value for "No" case
    ...
    if submit_btn and not st.session_state.get(f"phase{phase_num}_done", False):
        ...
        errors = []
        if decision is None:
            errors.append("Please make a decision regarding the user's behavior.")
        if decision_confidence is None:
            errors.append("Please select a Decision Confidence level.")
        if decision == "Potential Insider Threat":
            if scenario is None:
                errors.append("Please select a Scenario Type.")
            if scenario_confidence is None:
                errors.append("Please select a Scenario Confidence level.")
        
        if errors:
            for e in errors:
                st.error(e)
        else:
            # Processing valid submission
            ...
            try:
                ...
                if not correct_row.empty:
                    ...
                    if decision == "Not a Potential Insider Threat":
                         # Correct if scenario is 0 (Normal)
                         scenario_marks = 50 if correct_scenario == 0 else 0
                         insider_marks = 50 if correct_scenario == 0 else 0 
                    else:
                         # User said Threat
    ...
    if st.session_state.get(f"phase{phase_num}_done", False) and not st.session_state.get(f"questionnaire_p{phase_num}_done", False):
        ...
        # Helper with NO default (index=None)
        def radio_question(q, key):
            return st.radio(q, [1, 2, 3, 4, 5], index=None, key=key, horizontal=True)
        ...
        if st.button("Submit Questionnaire", key=f"submit_q_p{phase_num}"):
            ...
            all_questions = [q1, q2, q3, q4, q5, q6, q7, q8, q9]
            if any(a is None for a in all_questions):
                st.error("Please answer all rating questions (1-9) before submitting.")
            else:
                answers = all_questions + [q10]
                save_questionnaire_phase(int(st.session_state["student_id"]), int(user_number), phase_num, answers)

                trust = compute_trust_scores(int(st.session_state["student_id"]), int(user_number), phase_num, dict_df=dict_df)
                try:
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        # Removed Debug Dictionary from sidebar
    ...
    if generate_report:
        ...
        try:
            pdf_bytes = build_pdf_report_bytes_comparative(sid, u1, u2, dict_df=dict_df)
    ...
    if page_choice == "Instructions":
        st.markdown(
            ...
            "2. For each design (A and B), review the evidence:\n"
            "   - **Model Output**: (Open by default) Behavioral deviation logs.\n"
            "   - **Visual Analysis**: Anomaly Loss and CDF plots.\n"
            "   - **Input Logs**: Raw CSV data (activity logs).\n"
            "3. Make your decision, select scenario (only if Yes) and confidence.\n"
            "4. Submit and then complete the questionnaire.\n"
            "5. Generate the professional PDF report from the sidebar after completing both designs."
        )
